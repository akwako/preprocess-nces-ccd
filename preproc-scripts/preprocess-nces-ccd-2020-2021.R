...
YR <- substr(YEAR, 3, 4)
NCES_FOLDER <- paste0(WDIR, YEAR, '-', (YEAR+1), 'dst/')
DATE <- format(Sys.time(), '%Y%m%d')
FILE_TO_SAVE <- paste0(DATE, '_nces', YEAR, 'dst_cleaned.csv')
...
nces_dir[nchar(nces_dir$LEAID) == 6, 'LEAID'] <- 
  paste0('0', nces_dir[nchar(nces_dir$LEAID) == 6, 'LEAID']) # Convert all LEAIDs to 7-digits
...
#                         stringsAsFactors = F,
#                         na.strings=c("", " ", "#NULL!", "NA"))
# nces_memb[nchar(nces_memb$LEAID) == 6, 'LEAID'] <- 
#   paste0('0', nces_memb[nchar(nces_memb$LEAID) == 6, 'LEAID']) # Convert all LEAIDs to 7-digits
# 
...
#                                            'RNS' = 'Not Specified')
# 
# # Caculate total student enrollment 
...
#   as.data.frame()
# 
# # Caculate student enrollment by grade
...
#                          stringsAsFactors = F,
#                          na.strings=c("", " ", "#NULL!", "NA"))
# nces_staff[nchar(nces_staff$LEAID) == 6, 'LEAID'] <- 
#   paste0('0', nces_staff[nchar(nces_staff$LEAID) == 6, 'LEAID']) # Convert all LEAIDs to 7-digits
# 
# 
...
nces_disab <- read.csv(paste0(NCES_FOLDER, FILE_NCES_DISAB),
                       header = T,
                       stringsAsFactors = F,
                       na.strings=c("", " ", "#NULL!", "NA"))
nces_disab[nchar(nces_disab$LEAID) == 6, 'LEAID'] <- 
  paste0('0', nces_disab[nchar(nces_disab$LEAID) == 6, 'LEAID']) # Convert all LEAIDs to 7-digits
...
nces <- Reduce(function(x, y) merge(x, y, by = 'LEAID', all=TRUE),
                     list(nces_dir[, c('LEAID', 'LEA_NAME', 
                                       'LSTREET1', 'LSTREET2', 'LSTREET3', 'LCITY', 'LSTATE', 'LZIP', 
                                       'ST', 'STATE_AGENCY_NO', 'ST_LEAID', 
                                       'SY_STATUS_TEXT', 'LEA_TYPE_TEXT', 'CHARTER_LEA_TEXT', 'LEVEL', 'OPERATIONAL_SCHOOLS')],
                          # nces_memb_w,
                          # nces_staff[, c('NCESSCH', 'TEACHERS')], 
                          nces_disab[, c('LEAID', 'IDEA_COUNT')],
                          nces_geo[, c('LEAID', 'CNTY', 'NMCNTY', 'LAT', 'LON', 'LOCALE', 'CD', 'SLDL', 'SLDU')]))  # could add PCT_CITY13, etc.
...
# #                        levels = c('Not Virtual', 'Supplemental Virtual', 'Virtual with face to face options', 'Full Virtual'), 
...
#                   ifelse(nces$LSTATE %in% c('WA', 'OR', 'CA', 'MT', 'ID', 'WY', 'NV', 'UT', 'CO', 'AZ', 'NM'), 'west', 
